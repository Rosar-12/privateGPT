...
dev-windows:
	(set PGPT_PROFILES=ollama & poetry run python -m uvicorn private_gpt.main:app --reload --port 8001 --timeout-keep-alive 3600)

dev:
	PGPT_PROFILES=local poetry run python -m uvicorn private_gpt.main:app --reload --port 8001 --timeout-keep-alive 3600
