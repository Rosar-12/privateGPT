...
class IngestService:
    ...
    def ingest_audio_file(self, file) -> tuple[str, str]:
    ...
    def ingest_bin_data(
        ...
        if Path(file_name).suffix in ['.mp3', '.wav', '.flac', '.au', '.aiff', '.aif']:
            ingest_item_content = self.ingest_audio_file(raw_file_data)
            return self.ingest_text(Path(file_name).stem+'.txt', ingest_item_content)
        else:
